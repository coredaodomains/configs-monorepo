...
// check skip packages defined here.
...
function checkPeerDepedencies(packageJson) {
    ...
    if (packageJson.devDependencies === undefined) {
        console.error(`${packageJson.name} has peerDependencies but no devDependencies`)
...
function addDependencies(packageName, dependenciesToAdd, allDependenciesMap) {
    ...
    for (const [name, spec] of Object.entries(dependenciesToAdd)) {
        ...
        if (IGNORE_FROM_ALL[name] !== undefined && IGNORE_FROM_ALL[name].includes(packageName)) {
            continue
        }
...
function main() {
    ...
    for (const dependency of Object.keys(allDependenciesMap)) {
        if (Object.keys(allDependenciesMap[dependency]).length !== 1) {
            ...
            for (const [spec, packageNames] of Object.entries(allDependenciesMap[dependency])) {
